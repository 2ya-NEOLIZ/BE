 ...
 *
 * 두 가지 형태로 분기:
 *   - 정상 종료(abandoned=false): myResult(개인 결과+신기록) + ranking(주간 TOP5)
 *   - 비정상 종료(abandoned=true): score=0 + ranking=null
 *
 * 주간 랭킹(ranking)은 공통 RankingService 가 반환하는 WeeklyRankingResponse 를 재사용한다.
 */
@Getter
@AllArgsConstructor
@Builder
public class SubmitCatchResultResponse {
    ...
    /** 주간 랭킹 TOP5 + 본인 순위 (정상 종료 시) */
    private WeeklyRankingResponse ranking;

    /** 비정상 종료 시 점수 (0). 정상 종료 시 null */
    private Integer score;

    /** 정상 종료 응답 */
    public static SubmitCatchResultResponse of(MyResult myResult, WeeklyRankingResponse ranking) {
